...
// //*Aumentar y disminuir productos del carrito
const carPlus = document.querySelector(".increse");
const carMinus = document.querySelector(".decrece");
...
function eventListenersLoader() {
    ...
    // //*Cuando se preciona los botones + y -
    carPlus.addEventListener("click", IncreseProducts);
    carMinus.addEventListener("click",DecreceProducts );
    ...
    for (let product of carProducts){
        ...
        <div class="car__product">
            ...
            <div class="car__product__button">
                ...
                <button class="plus__product" data-id="${product.id}">
                +
                </button>

                <button class="minus__product" data-id="${product.id}">
               -
                </button>
            </div>
        </div>
...
function IncreseProducts(event){
    ...
    if(event.target.classList.contains('plus__product')){
        ...
        if(carProducts.some(product => product.id===productId)){

            const product_actual=carProducts.map(product => {
...
function DecreceProducts(event){
    ...
    if(event.target.classList.contains('minus__product')){
        const productId = event.target.getAttribute('data-id')
        if(carProducts.some(product => product.id===productId)){
...
//* Local Storage
//* Es una base datos del navegador que nos permite almacenr información para hacerla recurrente dentro de nuestra página.
...
//? convertir el objeto JavaScript en un JSON.
